...
cn <- connect.sql.server(database="", user='', domain="", server.address="", password='')
...
T1_column <- c('phewas_code','individual_idT1','GenderT1','SubscriberIdT1','ageAverageT1',
               'PheWAS_Indicator_T1', 'genderConfig','extraSSIDT1','monthEnrollmentT1','avgMonthlyVisitsT1','numComorbidT1' )
T2_column <- c('phewas_code','individual_idT2','GenderT2','SubscriberIdT2','ageAverageT2', 
               'PheWAS_Indicator_T2', 'genderConfig','extraSSIDT2','monthEnrollmentT2','avgMonthlyVisitsT2','numComorbidT2')

names_RF <- c('phewas_code','MemberId','Gender', 'SubscriberId', 'MemberBirthYear',  
              'Indicator', 'genderConfig', 'extraSSID', 'monthEnrollment', 'avgMonthlyVisits', 'numComorbid')


regression_column <- c('phewas_code', 'Indicator', 'MemberBirthYear', 'Gender',  
                       'SubscriberId','genderConfig', 'extraSSID','monthEnrollment', 'avgMonthlyVisits', 'numComorbid')


lmer_formula <- as.formula(Indicator ~  (1|SubscriberId) + (1|extraSSID) + MemberBirthYear + as.factor(Gender) + monthEnrollment + monthEnrollment^2 )
...
shared_environment <- function(tetraChorRhoSameSex, tetraChorRhoOppositeSex,p) {
  ...
  #csq <- 2 * ((1.5 * tetraChorRhoOppositeSex) - tetraChorRhoSameSex)
  ...
  while(bootIter < iterations)
  {
    ...
    num_1 <- nrow(d[d$Indicator==1,])
      ...
      data_h2_c2_row <- data_h2_c2_row %>% dplyr::select(rliabss_SE=rliabss,rliabos_SE=rliabos,
                                                  ...
                                                  rliabres_SE=rliabres, e2_liab_SE=e2_liab)
      if(bootIter == 0)
      {
...
SE_Estimates <- data.frame(rliabss_SE_se=sd(results$rliabss_SE), 
                           ...
                           e2_liab_SE_se=sd(results$e2_liab_SE))
...
filename <- paste0('phewas_',phewas_code,'.csv')

write.table(allData,filename, row.names = FALSE, col.names = FALSE)

saveRDS(names(allData),file='names_binary.rds')
